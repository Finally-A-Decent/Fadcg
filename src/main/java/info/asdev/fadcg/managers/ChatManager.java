...
public class ChatManager {
    ...
    @Getter
    private boolean centerFormat = false;
    ...
    public void init() {
        ...
        long interval = config.getLong("options.interval", 300_000);
        timeout = config.getLong("options.timeout", 60_000);
        minPlayers = config.getInt("options.min-players", 2);

        job = Job.of("chatreactions_run", this::runReaction, Duration.of(interval, ChronoUnit.MILLIS));
        job.start();
    }
    ...
    private void runReaction() {
        runSpecificReaction(ReactionManager.createReaction(), null, false);
    }

    public void runSpecificReaction(ReactionCategory category, @Nullable ChatGameImpl implementation, boolean force) {
    ...
    public void onPlayerEvent(Event event) {
        ...
        Player who = null;
        if (active instanceof ReactionUnscramble
                || active instanceof ReactionSolve
                || active instanceof ReactionReverse
                || active instanceof ReactionType
                || active instanceof ReactionFinishPhrase)
        {
            AsyncPlayerChatEvent ev = (AsyncPlayerChatEvent) event;
            who = ev.getPlayer();
            result = active.attempt(ev.getPlayer(), ev.getMessage(), event);
        } else if (active instanceof ReactionBlockBreak)
        {
            BlockBreakEvent ev = (BlockBreakEvent) event;
            who = ev.getPlayer();
            result = active.attempt(ev.getPlayer(), null, event);
        } else if (active instanceof ReactionBlockPlace)
        {
            BlockPlaceEvent ev = (BlockPlaceEvent) event;
            who = ev.getPlayer();
            result = active.attempt(ev.getPlayer(), null, event);
        } else if (active instanceof ReactionCraftItem)
        {
...
//        else if (active instanceof ReactionEat) {
//            PlayerItemConsumeEvent ev = (PlayerItemConsumeEvent) event;
//            who = ev.getPlayer();
//            result = active.attempt(ev.getPlayer(), null, event);
//        }
        else if (active instanceof ReactionKillMob)
        {
        ...
        } else if (active instanceof ReactionTrade)
        {
            PlayerTradeEvent ev = (PlayerTradeEvent) event;
            who = ev.getPlayer();
            result = active.attempt(ev.getPlayer(), null, event);
        }
//        else if (active instanceof PIGLIN_BARTER) {
